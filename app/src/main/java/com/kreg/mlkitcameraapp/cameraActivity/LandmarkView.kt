    ...
    ):View(context, attributes)
{
    ...
    private fun drawPoint(center: PointF3D, canvas: Canvas?)
    {
        ...
        canvas?.drawCircle(convertedPoint.x,convertedPoint.y,10f,mainPaint)
    }
    ...
    {
        var startPoint = detectedPose?.getPoseLandmark(PoseLandmark.RIGHT_SHOULDER)
        var endPoint = detectedPose?.getPoseLandmark(PoseLandmark.RIGHT_WRIST)
        if (startPoint != null && endPoint != null) {
            drawLine(startPoint.position3D, endPoint.position3D, canvas)
        }
        startPoint = detectedPose?.getPoseLandmark(PoseLandmark.LEFT_MOUTH)
